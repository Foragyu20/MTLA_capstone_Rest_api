...
if ($method === "POST") {
    ...
    $query = "INSERT INTO english_dictionary (word, part_of_speech,definition,example) VALUES (?, ?, ?, ?)";
...
if ($method === "PUT") {
    ...
    $query = "UPDATE english_dictionary SET word = ?, part_of_speech = ?, definition = ?, example = ? WHERE id = ?";
    $stmt = $connection->prepare($query);
...
if ($method === "DELETE") {
    ...
    $query = "DELETE FROM english_dictionary WHERE id = ?";
    $stmt = $connection->prepare($query);
