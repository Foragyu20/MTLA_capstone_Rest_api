...
if ($method === "POST") {
    ...
    $query = "INSERT INTO quizmed (question, option_1,option_2,option_3,option_4,correct_answer) VALUES (?, ?, ?, ?, ?, ?)";
...
if ($method === "PUT") {
    ...
    $query = "UPDATE quizmed SET question = ?, option_1 = ?, option_2 = ?, option_3 = ?, option_4 = ?, correct_answer = ? WHERE id = ?";
    $stmt = $connection->prepare($query);
...
if ($method === "DELETE") {
    ...
    $query = "DELETE FROM quizmed WHERE id = ?";
    $stmt = $connection->prepare($query);
