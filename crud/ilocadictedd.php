...
// Show all records
if ($method === "GET") {
    $query = "SELECT * FROM ilocano_dictionary";
...
if ($method === "POST") {
    ...
    $query = "INSERT INTO ilocano_dictionary (word, part_of_speech,definition,example) VALUES (?, ?, ?, ?)";
...
if ($method === "PUT") {
    ...
    $query = "UPDATE ilocano_dictionary SET word = ?, part_of_speech = ?, definition = ?, example = ? WHERE id = ?";
    $stmt = $connection->prepare($query);
...
if ($method === "DELETE") {
    ...
    $query = "DELETE FROM ilocano_dictionary WHERE id = ?";
    $stmt = $connection->prepare($query);
