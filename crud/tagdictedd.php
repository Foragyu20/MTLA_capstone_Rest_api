...
// Show all records
if ($method === "GET") {
    $query = "SELECT * FROM tagalog_dictionary";
...
if ($method === "POST") {
    ...
    $query = "INSERT INTO tagalog_dictionary (word, part_of_speech,definition,example,) VALUES (?, ?, ?, ?)";
...
if ($method === "PUT") {
    ...
    $query = "UPDATE tagalog_dictionary SET word = ?, part_of_speech = ?, definition = ?, example = ? WHERE id = ?";
    $stmt = $connection->prepare($query);
...
if ($method === "DELETE") {
    ...
    $query = "DELETE FROM tagalog_dictionary WHERE id = ?";
    $stmt = $connection->prepare($query);
